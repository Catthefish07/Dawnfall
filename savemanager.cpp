...
int SaveManager::generateUID(){
    int highestUID = 0;

    ifstream file(saveFile, ios::binary);
    if(!file){
         return highestUID + 1; // not saved, return 1 (first valid id)
    }

    for(int i = 0; i < totalSlots; i++){
        PlayerRecord p = {}; // give empty place first to hold data
        file.read((char*)&p, sizeof(PlayerRecord)); // reads data
        if(p.occupied){ // reads occupied
            int id = stoi(string(p.UID)); // convert char to string then to int
            if(id > highestUID){
                highestUID = id;
            }
        }
    }
    file.close();           // occupied if false -> skips and make new highest ID
    return highestUID + 1; // return new highest ID
}


void SaveManager::saveGame(int slot, PlayerRecord player, Inventory& iv, Shop& shop, PartyManager& pm){
    ...
    vector<Character*> allCharacters = pm.getUnlockedCharacters();
    // ensures unlocked charaacters in party and saves level
    for(int i = 0; i < allCharacters.size(); i++){
        player.characterLevels[i] = allCharacters[i]->getLevel();
    }

    saveInventory(iv); // saves items amount

    vector<Character*> party = pm.getParty();
    for(int i = 0; i < 4; i++){
        player.partyIndexes[i] = -1; // default empty or implies no character
        if(i < party.size()){ // to check party size
            for(int j = 0; j < allCharacters.size(); j++){ // checks all characters unlocked
                if(allCharacters[j] == party[i]){ // if true (in the party)
                    player.partyIndexes[i] = j; // then is in the party and saves to file
                    break;
                }
            }
        }
...
PlayerRecord SaveManager::loadGame(int slot, Inventory& iv, Shop& shop, PartyManager& pm){
    PlayerRecord player = {};
    ...
    vector<Character*> allCharacters = pm.getUnlockedCharacters(); // ensures that the ones goes in the party are unlocked ones
    for(int i = 0; i < allCharacters.size(); i++){ // restore levels of unlocked characters
        allCharacters[i]->setLevel(player.characterLevels[i]);
    }

    pm.clearParty();
    for(int i = 0; i < 4; i++){
        int index = player.partyIndexes[i]; // read from file (the index)
        if(index != -1 && index < allCharacters.size()){
            pm.addCharacter(allCharacters[index]); // addCharacter() checks is unlocked too from partymanager
        }                                          // then add to party if yes
    }
