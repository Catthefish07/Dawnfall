...
int SaveManager::generateID(){
    int highestID = 0;

    ifstream file(saveFile, ios::binary);
    if(!file){
         return highestID + 1; // not saved, return 1 (first valid id)
    }

    for(int i = 0; i < totalSlots; i++){
        PlayerRecord p = {}; // give empty place first to hold data
        file.read((char*)&p, sizeof(PlayerRecord)); // reads data
        if(p.occupied && p.playerID > highestID){
            highestID = p.playerID; // reads occupied and highest ID
        }                           // occupied if false -> skips and make new highest ID
    }
    file.close();
    return highestID + 1; // return new highest ID
}
