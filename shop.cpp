...
bool Shop::upgradeCharacter(string characterName, int& playerCoins, PartyManager& pm){
    for(CharacterShop& c : characterStock){
        if(c.name == characterName){
            if(!c.isOwned){ // ensures only upgrade character that player owns
                cout << "Character not owned!" << endl;
                return false;
            }
            ...
            for(Character* ch : allCharacters){
                if(ch->getName() == characterName){
                    int upgradeCost = ch->getLevel() * 200; // cost scales with level
                    ...
                    ch->levelUp();
                    cout << characterName << " upgraded to level " << ch->getLevel() << "!" << endl;
                    return true;
                }
            }
        }
    }
    cout << characterName << " not found!" << endl;
    return false;
}
